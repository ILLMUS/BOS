...
export default function DocumentPreviewDialog({ open, onOpenChange, payload, jobId, clientPhone, clientEmail, onStored }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setMessage(
        ...
        `totalling ${money(total, payload.currency)} from ${from.name}.`,
      );
    })();
    return () => {
      active = false;
  ...
  const sendWhatsApp = async () => {
    ...
    try {
      ...
      if (payload.kind === "quote" && jobId) {
        void notifyQuoteEvent(
          jobId,
          "sent",
          `Quotation ${payload.number} sent to ${payload.to.name || "the customer"}`,
          `${label} ${payload.number} totalling ${money(total, payload.currency)} was shared on WhatsApp.`,
        );
      }
    } catch (e: any) {
  ...
  const sendEmail = async () => {
    ...
    try {
      const link = await storeAndLink();
      const { data, error } = await supabase.functions.invoke("send-client-document", {
        body: {
          recipientEmail: to,
          ...
          amount: money(total, payload.currency),
          message,
          documentUrl: link,
        },
      });
      ...
      if (data && (data as any).success === false) {
        throw new Error("This client has opted out of emails from us");
      }
      ...
      if (payload.kind === "quote" && jobId) {
        void notifyQuoteEvent(
          ...
          "sent",
          `Quotation ${payload.number} sent to ${payload.to.name || "the customer"}`,
          `${label} ${payload.number} totalling ${money(total, payload.currency)} was emailed to ${to}.`,
        );
      }
    } catch (e: any) {
