...
/** Finds the niche key behind a stored industry label (set during setup). */
export function nicheKeyFromIndustry(industry?: string | null, presets?: { key: string; label: string }[]) {
  if (!industry) return null;
  const needle = industry.trim().toLowerCase();
  const hit = (presets || []).find((p) => p.label.toLowerCase() === needle || p.key === needle);
  return hit?.key ?? null;
}

/**
 * Swaps the neutral defaults inside a fixed piece of copy (nav labels, page
 * titles) for the wording this workspace chose. Case of the first letter is kept.
 */
const SUBSTITUTIONS: [CopyTerm, string][] = [
  ["work_items", "Jobs"],
  ["work_item", "Job"],
  ["workflows", "Workflows"],
  ["workflow", "Workflow"],
...
export function applyCopy(text: string, map: CopyMap): string {
  let out = text;
  for (const [term, fallback] of SUBSTITUTIONS) {
    const replacement = map[term];
    if (!replacement || replacement.toLowerCase() === fallback.toLowerCase()) continue;
    const re = new RegExp(`\\b${fallback.replace(/ /g, "\\s")}\\b`, "gi");
    out = out.replace(re, (matched) =>
      matched[0] === matched[0].toUpperCase() ? replacement : lower(replacement),
    );
  }
  return out;
}
