...
import { Filter, Loader2, Plus, TicketIcon } from "lucide-react";
    ...
    <div className="space-y-5 text-slate-200">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.065] pb-4">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-400">
            Client issues, warranty calls, and incoming requests managed in one live queue
          </p>
        </div>

        {/* LOG NEW TICKET DIALOG */}
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="h-8 rounded-lg bg-cyan-500 px-3.5 text-[11px] font-bold text-slate-950 shadow-[0_0_20px_rgba(34,211,238,0.2)] transition-all hover:bg-cyan-400 hover:shadow-[0_0_25px_rgba(34,211,238,0.35)] active:scale-[0.98]">
              <Plus className="mr-1.5 h-3.5 w-3.5" /> Log Ticket
            </Button>
          </DialogTrigger>
          <DialogContent className="border-white/[0.085] bg-[#10151d] text-slate-200 shadow-[0_18px_60px_rgba(0,0,0,0.4)] sm:max-w-lg">
            ...
            <div className="space-y-3 pt-2">
              ...
              />
              <Textarea
                placeholder="Describe the client request or issue..."
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Total Tickets", value: counts.total, valColor: "text-slate-100" },
        ].map((k) => (
      ...
      {/* QUEUE MAIN GLASS CARD */}
      <GlassCard
        title="Active Support Queue"
        subtitle="Manage client status, priorities, and resolution notes"
        action={
          <div className="flex items-center gap-2">
            <Filter className="h-3.5 w-3.5 text-slate-400" />
            <Select value={filter} onValueChange={setFilter}>
              <SelectTrigger className="h-8 w-40 rounded-xl border-white/[0.08] bg-[#0b0e14] text-[11px] text-slate-300 focus:border-cyan-400/40">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="border-white/[0.085] bg-[#10151d] text-slate-200">
        ...
        <div className="p-6 space-y-3">
          {loading ? (
            ...
            <p className="py-6 text-center text-[11px] text-slate-500">
              No tickets recorded for this filter configuration.
            </p>
          ) : (
            visible.map((t) => (
              <div
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] text-slate-400">
                      {accountName(t.account_id) || "No account linked"} · {label(t.category)} ·{" "}
                      <span className="font-mono text-slate-300">{formatDateTime(t.created_at)}</span>
                    </p>
                  </div>

                  <div className="flex items-center gap-2">
                    <span
                      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider ${getPriorityStyle(
                        t.priority
                      )}`}
                    >
                      {label(t.priority)}
                    ...
                    >
                      <SelectTrigger className="h-7 w-36 rounded-lg border-white/[0.08] bg-[#0b0e14] text-[10px] text-slate-200 focus:border-cyan-400/40">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="border-white/[0.085] bg-[#10151d] text-slate-200">
                        {TICKET_STATUSES.map((s) => (
                ...
                <Textarea
                  className="min-h-[60px] rounded-xl border-white/[0.08] bg-[#0b0e14] text-[11px] text-slate-200 placeholder:text-slate-500 focus:border-cyan-400/40 focus:ring-1 focus:ring-cyan-400/40"
                  placeholder="Enter resolution details or internal updates..."
