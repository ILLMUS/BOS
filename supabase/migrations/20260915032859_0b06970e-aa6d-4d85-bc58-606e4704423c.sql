...
BEGIN
  ...
  IF _existing.id IS NULL AND _ref IS NOT NULL THEN
    SELECT * INTO _existing FROM finance_documents
     WHERE org_id = _job.org_id AND doc_type = _kind AND reference = _ref LIMIT 1;
  END IF;
  IF _existing.id IS NULL THEN
    -- adopt a document already recorded for this job by another source
    SELECT * INTO _existing FROM finance_documents
     WHERE org_id = _job.org_id AND doc_type = _kind AND job_id = _job.id
       AND (external_id IS NULL OR external_id NOT LIKE 'stage:%')
     ORDER BY created_at LIMIT 1;
  END IF;

  _ref := COALESCE(_ref, _existing.reference,
    CASE _kind WHEN 'quote' THEN 'QT-' WHEN 'invoice' THEN 'INV-' ELSE 'REC-' END || COALESCE(_job.job_number,'DRAFT'));
...
-- merge the duplicate workflow quotes into the originals
DELETE FROM public.finance_documents
WHERE reference IN ('QT-LCS-00002','QT-LCS-00003') AND source = 'workflow';

UPDATE public.job_stages SET stage_name = stage_name
